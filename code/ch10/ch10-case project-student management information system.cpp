...
void save(struct student *head) //保存文件函数
{
	FILE *fp1;
	struct student *p1,*p2;
	p1=p2=head;
	if((fp1=fopen("e:\\data\\a.dat","wb"))==NULL)
	{
...
struct student *open()  //打开文件函数
{
	FILE *fp1;
	struct student *p1,*p2,*head=NULL;
	n=0;
	if((fp1=fopen("e:\\data\\a.dat","rb"))==NULL)
	{
		printf("can not open file\n");
		exit(0);
	}
	head=p2=p1=(struct student*)malloc(sizeof(struct student));
	fread(p1,sizeof(struct student),1,fp1);
	while(p1->next!=NULL)
	{
		n++;
		if(n==1) head=p1;
...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		switch(ch){
			...
			case '9':	head=open();display(head);break; 
			case '0': 	exit(0);         					         
		}		
		printf("\n按任意键继返回菜单,按Q退出.....\n");
		ch=getch();
