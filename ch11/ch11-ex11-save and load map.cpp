...
void save_map(int map[MAP_ROW][MAP_COL], const char* file_name)  
{  
	FILE *fp;       /* 1、定义文件指针 FILE *fp  */  
	/* wt 表示以文本模式写入文件 */   
	if((fp = fopen(file_name, "wt"))==NULL)  /* 2、打开文件 fopen()  */  
	{  
	...
	fclose(fp); /* 3、关闭文件  fclose()*/       
} 

void load_map(int map[MAP_ROW][MAP_COL], const char* file_name)  
{  
	FILE *fp;      /* 1、定义文件指针 FILE *fp  */  
	/* rt 表示以文本模式读文件 */   
	if((fp = fopen(file_name,"rt"))==NULL)  /* 2、打开文件 fopen()  */  
	{  
		printf("File open error!\n");  
	}  
	for(int i = 0; i < MAP_ROW; i++)
	...
	while(1)
	{
		...
		process_key_events(&iRobot_x, &iRobot_y, &iFlag_x, &iFlag_y, 
                        &iPlace_flag, iRobot_vx, iRobot_vy,
		                 treasure_x, treasure_y, treasure_type, iMap);			
